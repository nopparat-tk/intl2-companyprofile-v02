...
export default function ContactFormWithGclid() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Prepare the data as a JSON object, including the gclid
    const dataToSubmit = {
      Name: formData.name,
      Email: formData.email,
      Phone: formData.phone,
      "Line ID": formData.idline,
      Message: formData.massege,
      gclickid: formData.gclid, // Ensure the key matches your Google Sheet header
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // It's often better to check response status for non-ok responses.
        throw new Error(
